...
class GPSConnection(object):
    ...
    def connectPort(self):
        """
        Method cycles through port numbers from 0 to 10 and baud rates shown below to attempt
        ...
        self.port = 0
        self.portName = ''
        # moved to init to enable at start: self.baudRates = [1200, 2400, 4800, 9600, 14400, 28800, 36400, 56700]
        while(not self.connected and self.port < 11):
            for self.baudRate in self.baudRates:
                ...
                if self.port > 10:
                    ...
                    self.port += 1
            if self.connected: break # got connected, break out of while loop
        if self.connected: self.portName = self.serialPort.portstr
        else: raise NoGPSConnected("Could not find a GPS Serial Connection sending NMEA messages!")
    ...
    def connectPortBySettings(self,portNumber,portSpeed):
        ...
        self.tryConnectPortSpeed(self.port,self.baudRate)
        if self.connected: self.portName = self.serialPort.portstr
        else: raise NoGPSConnected("Could not find a GPS Serial Connection sending NMEA messages on port: COM" +
    ...
    def tryConnectPortSpeed(self,port,baudRate):
        ...
        # print 'Just tried port:' + str(port)
        except: pass # some other exception in connecting
        ...
        for i in range(20): # look at 20 lines to acquire all of the data
            ...
            if aline[:6] == '$GPGGA':
                try:
                    ...
                    if hasBearing and hasSatellites: break
                except Exception, e:
                ...
                try:
                    ...
                    hasBearing = True
                    if self.hasFix and hasSatellites: break
                except Exception, e:
                ...
                try:
                    ...
                    for iSat in range(numSatInMess):
                        ...
                        if iSat == numSatInMess - 1: # special processing for last satellite in message - remove checksum
                            ...
                            thePosition.satelliteData.append((parts[theIndex+4], parts[theIndex+5], parts[theIndex+6],\
                                                   parts[theIndex+7]))
                    if parts[1] == parts[2]: hasSatellites = True # have processed last satellite message
