...
class GPSTrackerOptions(QDialog, gpsTrackingOptionsDlg):
    ...
    def __init__(self, trackerObject, parent=None):
        ...
        for i in range(len(self.trackerObject.read.session.baudRates)):
            self.cbxPortSpeed.addItem(str(self.trackerObject.read.session.baudRates[i]),QVariant(i))

        # Load the ports selection box
        for i in range(11):
            pName = 'COM' + str(i+1)
            self.cbxPortName.addItem(pName,QVariant(i))
        ...
        self.cbxMarkerType.addItem(QIcon(os.path.join(globalpath,"images\\triangle.png")),"Notched Triangle",QVariant(0))
        self.cbxMarkerType.addItem(QIcon(os.path.join(globalpath,"images\\wideArrow.png")),"Wide Arrow",QVariant(1))
        self.cbxMarkerType.addItem(QIcon(os.path.join(globalpath,"images\\bluntArrow.png")),"Blunt Arrow",QVariant(2))
        self.cbxMarkerType.addItem(QIcon(os.path.join(globalpath,"images\\pointedStick.png")),"Pointed Stick",QVariant(3))
